...
/*   CODE TO RESTORE FOOTER POSITION
     AURTHOR :AMEEDUDDIN
 */
...
{    
    console.log('TYPE ameed= '+user.Type);
    if(loginstatus==0)
    {
        //alert('2')
        $.mobile.changePage("#pageLogin", {transition:"none"})
...
function navToprevwlst()
{
    $.mobile.changePage("#pageprevwListing", {transition:"none"})
}
function navPayment()
{
  //  alert(user.Type+"  "+truckInfo.payment_status+"  "+truckInfo.detailsAddListing);
    if(user.Type=="Registered" )
    {
        ...
        else if(truckInfo.detailsAddListing=='1'&&truckInfo.payment_status==1){
            $.mobile.changePage("#pageDashboard", {transition:"none"})
        }
        else if(truckInfo.detailsAddListing=='1'&&truckInfo.payment_status==0){
            $.mobile.changePage("#pagePayment", {transition:"none"})
        }
        else
        {
            if(truckInfo.payment_status!='1')
            {
                $.mobile.changePage("#pagePayment", {transition:"none"})
            }
            else
